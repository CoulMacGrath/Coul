...
class Builder:
    ...
    def get_column(self,table=None, client=None):
        if client != None:
            self.client = client
        if table != None:
            self.table = table
        ...
        self.query = self.client.query('select count(case_id), cast(round(max(diff)) as smallint),'
                                       ...
                                       'from ' + self.table + ' group by case_id))) as a from (select case_id, '
                                       'date_diff(second, min(start_time), max(end_time))/(60*60*24) as diff '
                                       'from ' + self.table + ' group by case_id order by diff) group by a order by a')
    ...
    def get_variables(self, filter_column=None, table=None, client=None):
        if client != None:
            self.client = client
        if table != None:
            self.table = table
