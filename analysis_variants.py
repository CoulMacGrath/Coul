...
class Request:
    ...
    def __init__(self, id, client, request):
        self.id = id
        self.client = client
...
def calculate_variables():
    ...
    #Получение данных
    countid_groupby_time = client.query('select a, count(case_id)  from (select case_id, arrayMap((x)->x[1], arraySort((x)->toDateTime(x[2]), groupArray([activity, toString(start_time)]))) as a  from main_table group by case_id)'
                                        'group by case_id) group by a order by count(case_id) desc')
